...
| 파일 | 역할 |
|---|---|
| `config.json` | sources.meta(advertisers: label·page_id·type·kbeauty) / sources.tiktok(industry·regions·period) + 뷰티 tag_enums + output_dir |
...
## 대상 (config.sources)
- **Meta advertisers**: label·page_id·type(retailer/brand)·kbeauty. page_id 는 광고주의 **글로벌 페이지 id**(Ad Library URL 의 `view_all_page_id`). 현재 Sephora·Nocibé 적재, 나머지 22개(YesStyle·Aroma-Zone·Douglas·Notino·Marionnaud·Lookfantastic·Olive Young Global·Stylevana + K뷰티 14)는 page_id 받는 대로 추가.
