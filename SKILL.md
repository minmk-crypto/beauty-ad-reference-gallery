...
| 파일 | 역할 |
|---|---|
| `config.json` | sources.meta(advertisers: label·page_id·type·kbeauty + advertisers_sheet.csv_url) / sources.tiktok(industry·regions·period) + 뷰티 tag_enums + output_dir |
| `sync-advertisers.js` | 마케터 관리 Google Sheet(CSV publish) → config.sources.meta.advertisers 동기화. URL=env `ADVERTISERS_SHEET_CSV_URL` > config.csv_url. fetch 실패·빈 결과 시 기존 config 유지(fail-safe, exit 0). 갱신 주기 첫 단계 |
