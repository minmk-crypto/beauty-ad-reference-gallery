...
function loadNew() {
  ...
  for (const f of ['manifest-meta.json', 'manifest-tiktok.json']) {
    ...
    if (m && Array.isArray(m.new)) for (const r of m.new) out.push(r);
  }
  return out;
}
...
function runClaude(batchPath, outPath) {
  const r = spawnSync('claude', ['-p', PROMPT(batchPath, outPath), '--allowedTools', 'Read,Write,Glob'],
...
(async () => {
  ...
  const all = loadNew();
  const todo = all.filter(r => { const k = `${r.source}:${r.ad_id}`; return !(tags[k] && tags[k].summary); });
