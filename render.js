...
.pmedia{flex:none;width:100%;border-radius:11px;overflow:hidden;background:#0c0d10;border:1px solid var(--line);text-align:center;font-size:0}.pmedia img{max-width:100%;max-height:66vh;width:auto;height:auto;display:inline-block;vertical-align:middle;object-fit:contain}.pmedia video{max-width:100%;max-height:66vh;display:inline-block}
...
function card(a,i){
  ...
  return '<article class="card'+(a.active?'':' off')+'" data-i="'+i+'" tabindex="0">'+
    ...
    '<div class="cbody">'+
      '<div class="tagrow"><span class="advtag" style="color:'+a.bc+';background:color-mix(in srgb,'+a.bc+' 13%,transparent)">'+esc(a.adv)+'</span>'+(a.kb?'<span class="kbtag">K</span>':'')+'</div>'+
      (a.summary?'<div class="title">'+esc(a.summary)+'</div>':'')+
      '<div class="excerpt">'+esc(a.copy)+'</div>'+
      '<div class="pills">'+pills(a)+'</div>'+
      '<div class="cmeta">'+(a.started?'<span>'+esc(a.started)+'</span>':'')+(a.ctries.length?'<span class="sep">·</span><span>'+a.ctries.slice(0,6).join(' ')+(a.ctries.length>6?'+':'')+'</span>':'')+(a.ctr?'<span class="sep">·</span><span>CTR '+a.ctr+'</span>':'')+'</div>'+
    '</div></article>';
}
...
function openPeek(i){if(i<0||i>=view.length)return;cur=i;const a=ADS[view[i]];
  ...
  const media=a.video?'<video src="'+esc(a.video)+'" controls autoplay muted loop playsinline></video>':(a.media?'<img src="'+a.media+'" alt="">':'<div style="min-height:280px;display:flex;align-items:center;justify-content:center;color:#888">미리보기 없음</div>');
  ...
  $('#pbody').innerHTML='<div class="pmedia">'+media+'</div>'+
    ...
    '<dl class="prop">'+
      ...
      (a.ctries.length?'<dt>국가</dt><dd class="mono">'+a.ctries.join(' ')+'</dd>':'')+
    ...
    '<div class="plinks">'+(a.video?'<a class="primary" href="'+esc(a.video)+'" target="_blank" rel="noopener">영상 원본</a>':'')+(a.detail?'<a href="'+esc(a.detail)+'" target="_blank" rel="noopener">'+(a.src==='meta'?'Ad Library':'Creative Center')+'</a>':'')+(a.landing?'<a href="'+esc(a.landing)+'" target="_blank" rel="noopener">랜딩</a>':'')+'</div>';
...
$('#grid').addEventListener('click',e=>{const c=e.target.closest('.card');if(!c)return;const a=ADS[+c.dataset.i];if(e.target.closest('.vclose')){e.stopPropagation();stopInline(c);return;}if(e.target.closest('.pbtn')){e.stopPropagation();playInline(c,a);return;}const p=view.indexOf(+c.dataset.i);if(p>=0)openPeek(p);});
